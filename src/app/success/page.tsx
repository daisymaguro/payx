...
export default function SuccessPage() {
  ...
  const newTotal = 340 + starsEarned;
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: "#F2F0EB" }}>
      ...
      {/* Green top band */}
      <div className="relative overflow-hidden px-5 pt-16 pb-24"
        ...
        <div className="flex flex-col items-center">
          <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-5 transition-all duration-700
              ${show ? "pop-in" : "scale-0 opacity-0"}`}
            style={{ background: "linear-gradient(135deg, #CBA258, #F1D592)", boxShadow: "0 0 40px rgba(203,162,88,0.4)" }}>
            <svg width="44" height="44" viewBox="0 0 44 44" fill="none">
              <path d="M10 22L18 30L34 14" stroke="white" strokeWidth="3.5"
                strokeLinecap="round" strokeLinejoin="round"
                className={show ? "checkmark-path" : ""} />
            </svg>
          </div>
          <h1 className="text-white text-2xl font-black text-center">ชำระเงินสำเร็จ!</h1>
      ...
      <div className="px-4 -mt-12 relative z-10 space-y-3 pb-28">
        ...
        <div className="bg-white rounded-2xl p-4" style={{ boxShadow: "0 4px 20px rgba(30,57,50,0.12)" }}>
          <div className="flex items-center justify-between mb-4 pb-4"
            ...
            <div>
              ...
              <p className="font-black text-2xl tracking-widest" style={{ color: "#1E3932" }}>
                {orderId}
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl p-4" style={{ boxShadow: "0 2px 12px rgba(30,57,50,0.06)" }}>
          <p className="font-bold text-xs uppercase tracking-wide mb-4" style={{ color: "#006241" }}>
            สถานะออเดอร์
          </p>
            ...
            {steps.map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: step.done ? "#006241" : "#9CA3AF" }}>
                    {step.time}
                  </p>
                </div>
                {step.done && (
        ...
        <div className={`bg-white rounded-2xl p-4 ${show ? "stars-pop" : "opacity-0"}`}
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-400 text-xs mt-0.5">จากออเดอร์นี้</p>
            </div>
            <span className="font-black text-2xl" style={{ color: "#CBA258" }}>+{starsEarned}</span>
          </div>
          <div>
            <div className="flex justify-between text-xs mb-1.5">
              <span className="text-gray-400">ยอด Stars ใหม่</span>
              <span className="font-bold" style={{ color: "#CBA258" }}>{newTotal} ★</span>
            </div>
            <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "#F2F0EB" }}>
              <div className="h-full rounded-full"
                style={{ width: `${Math.min((newTotal / 400) * 100, 100)}%`,
                  background: "linear-gradient(90deg, #CBA258, #F1D592)" }} />
            </div>
            <p className="text-xs text-gray-400 mt-1">อีก {400 - newTotal} Stars → รับเครื่องดื่มฟรี</p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm bg-white px-4 py-5"
        ...
        <div className="flex gap-3">
          <button onClick={() => { dispatch({ type: "CLEAR_CART" }); router.push("/"); }}
            ...
            <span className="relative">หน้าหลัก</span>
          </button>
