...
import { ArrowLeft, Trash2, Plus, Minus, ChevronRight, Pencil } from "lucide-react";
...
export default function CartPage() {
    ...
    return (
      <div className="flex flex-col min-h-screen" style={{ background: "#F2F0EB" }}>
        <div className="flex items-center gap-3 px-5 pt-14 pb-4 bg-white">
          ...
          <h1 className="font-bold text-gray-800 text-base">ตะกร้า</h1>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
          <div className="text-7xl">☕</div>
          <h2 className="font-bold text-gray-700 text-lg">ตะกร้าว่างเปล่า</h2>
          <p className="text-gray-400 text-sm text-center">เลือกเครื่องดื่มที่คุณชอบก่อนนะครับ</p>
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: "#F2F0EB" }}>
      ...
      <div className="flex-1 overflow-auto pb-36 px-4 py-4 space-y-3">
        ...
        <div className="bg-white rounded-2xl p-4" style={{ boxShadow: "0 2px 12px rgba(30,57,50,0.06)" }}>
          ...
          <div className="mt-3 pt-3 flex items-center gap-2 rounded-xl px-3 py-2"
            style={{ background: "#D4E9E2" }}>
            <span style={{ color: "#CBA258" }}>★</span>
            <p className="text-xs font-semibold" style={{ color: "#1E3932" }}>
              คุณจะได้รับ <span style={{ color: "#CBA258" }}>+{starsEarned} Stars</span> จากออเดอร์นี้
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm bg-white px-4 py-5"
        ...
        <button onClick={() => router.push("/payment")}
          ...
          <span className="relative font-bold">ดำเนินการชำระเงิน</span>
